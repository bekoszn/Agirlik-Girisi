...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
        ...
        do {
            ...
            if sonuclar.count > 0 {
                for sonuc in sonuclar as! [NSManagedObject] {
                    if let isim = sonuc.value(forKey: "isimyeni") as? String {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ikinciVC" {
            ...
            destinationVC.secilenHareketIsmi = secilenHareket
            destinationVC.secilenHareketUUID = secilenUUID
        }
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        secilenHareket = isimDizisi[indexPath.row]
